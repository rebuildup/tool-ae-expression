...
tool-ae-expression の current release-driven development で使用する Operating Model を定義します。
...
- host repository の work state をこの repository へ canonical copy しない
...
- `src/` is owned here; my-web-2025 host and external UI are not.
- standalone checkout cannot prove full integration; host-layout validation is a separate evidence stage.
- Existing project ADRs / AGENTS host-boundary decisions remain more specific than replaceable upstream Practices.
- Bun is only Agent Skills tooling here unless project-local toolchain docs explicitly adopt it for source validation.

## Agent Skills lifecycle

- current `rebuildup/project-init` is installed/updated via `bunx skills`.
